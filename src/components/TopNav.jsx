...
export default function TopNav() {
  ...
  return (
    <nav className="relative w-full bg-transparent text-white">
      {/* Wrapper positions nav in top-right */}
      <div className="absolute top-5 right-6 z-50">
        {/* Desktop nav: visible links on md+ */}
        <div className="hidden md:flex items-center gap-1">
          <Link
            href="/about-us"
            className="px-3 py-1.5 text-sm font-semibold text-gray-200 hover:text-white hover:bg-white/5 rounded-md transition"
          >
            About
          </Link>
          <Link
          ...
          <Link
            href="/"
            className="ml-1 px-3 py-1.5 text-sm font-semibold text-white border border-white/15 hover:bg-white/10 rounded-md transition"
          >
            Use the App
          </Link>
        </div>

        {/* Mobile nav: hamburger + dropdown, only below md */}
